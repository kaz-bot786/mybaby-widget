...
# ── Inject API key from environment (set via GitHub Secret) ─────────────────
import os

def inject_api_key(html_path):
    api_key = os.environ.get("ANTHROPIC_API_KEY", "")
    if not api_key:
        print(f"WARNING: ANTHROPIC_API_KEY not set, skipping key injection for {html_path}")
        return
    with open(html_path, "r") as f:
        content = f.read()
    if "%%ANTHROPIC_API_KEY%%" in content:
        content = content.replace("%%ANTHROPIC_API_KEY%%", api_key)
        with open(html_path, "w") as f:
            f.write(content)
        print(f"API key injected into {html_path}")
    else:
        print(f"No placeholder found in {html_path}, skipping")
